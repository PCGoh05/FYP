...
Respond with ONLY the classification type, nothing else."""
    ...
    def get_writing_suggestions(self, text: str, paragraph_type: str = "body") -> List[str]:
        """
        Get writing suggestions for a paragraph
        ...
        Returns:
            List of suggestions
        """
        if not self._available or len(text) < 30:
            return []
        
        prompt = f"""As an academic writing expert, review this {paragraph_type} paragraph and provide improvement suggestions:
...
Provide 1-2 specific, actionable suggestions. Be brief and constructive."""

        system_prompt = "You are an academic writing expert providing brief, constructive feedback."
    ...
    def generate_summary(self, issues: List[Dict], changes: List[Dict]) -> str:
        """
        Generate an intelligent summary of the format check results
        ...
        """
        if not self._available:
            return self._fallback_summary(issues, changes)
        
        issue_summary = "\n".join([
            f"- {issue.get('description', 'Issue')} at {issue.get('location', 'Unknown')}"
            for issue in issues[:10]
        ])
        
        change_summary = "\n".join([
            f"- Changed {change.get('location', 'Unknown')}: {change.get('before', '')} → {change.get('after', '')}"
            for change in changes[:10]
        ])
        
        prompt = f"""Summarize these manuscript formatting check results in 2-3 sentences:

Issues Found ({len(issues)} total):
{issue_summary}

Changes Made ({len(changes)} total):
{change_summary}

Provide a brief, encouraging summary highlighting the main areas that needed correction."""
    ...
    def _fallback_summary(self, issues: List[Dict], changes: List[Dict]) -> str:
        """Fallback summary when LLM unavailable"""
        return (
            f"The format check found {len(issues)} issues in your manuscript. "
            f"{len(changes)} corrections have been applied automatically. "
            f"Please review the changes to ensure they meet your expectations."
        )
